...
export class SettingsTab extends PluginSettingTab {
  ...
  setup_custom_regexp(note_type: string, row_cells: HTMLCollection) {
    ...
    let custom_regexp = new Setting(row_cells[1] as HTMLElement).addText(text =>
      text
  ...
  setup_link_field(note_type: string, row_cells: HTMLCollection) {
    ...
    let link_field = new Setting(row_cells[2] as HTMLElement).addDropdown(async dropdown => {
  ...
  setup_context_field(note_type: string, row_cells: HTMLCollection) {
    ...
    let context_field = new Setting(row_cells[3] as HTMLElement).addDropdown(async dropdown => {
      const field_names = plugin.fields_dict[note_type]
      for (let field of field_names) {
        dropdown.addOption(field, field)
      }
    ...
    for (let key of Object.keys(plugin.settings['Defaults'])) {
      ...
      if (typeof plugin.settings['Defaults'][key] === 'string') {
        new Setting(defaults_settings)
          ...
          .addText(text =>
            text.setValue(plugin.settings['Defaults'][key]).onChange(value => {
        ...
        new Setting(defaults_settings)
          ...
          .setDesc(defaultDescs[key])
          .addToggle(toggle =>
            toggle.setValue(plugin.settings['Defaults'][key]).onChange(value => {
              plugin.settings['Defaults'][key] = value
              plugin.saveAllData()
            })
          )
  ...
  setup_ignore_files() {
    ...
    new Setting(ignored_files_settings)
      ...
      .addTextArea(text => {
        text
          .setValue(plugin.settings.IGNORED_FILE_GLOBS.join('\n'))
