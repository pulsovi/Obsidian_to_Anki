    ...
    return new Promise((resolve, reject) => {
        ...
        xhr.addEventListener('load', () => {
            try {
                ...
                logAnkiRequest(action, params, response);
...
function logAnkiRequest (action: string, params, response) {
    if (
      response.error ||
      action !== 'multi' ||
      !Array.isArray(params.actions) || !Array.isArray(response.result) || params.actions.length !== response.result.length
    ) {
        console.info('AnkiRequest', { action, params, ...response });
        return;
    }
    for (let i = 0; i < params.actions.length; ++i) {
        const request = params.actions[i];
        logAnkiRequest(request.action, request.params, response.result[i]);
    }
}
